...
//      ((*selectedLeptons)[0]->p4_ + (*selectedLeptons)[1]->p4_).M()<20) {
    ...
    if(selectedLeptonsMR->size()<=3 ){
      ...
      for (int l=0;l<selectedLeptonsMR->size();l++){
        delete (*selectedLeptonsMR)[l];
        delete (*selectedLeptonsMR_copy)[l];
      }
    selectedLeptonsMR->clear();
    selectedLeptonsMR->shrink_to_fit();
    delete selectedLeptonsMR; 
    selectedLeptonsMR_copy->clear();
    selectedLeptonsMR_copy->shrink_to_fit();
    delete selectedLeptonsMR_copy;
    continue;
    }
    ...
    if(selectedLeptonsAR->size() <3 || selectedLeptons->size()!=2 ){
      ...
      for (int l=0;l<selectedLeptonsAR->size();l++){
        delete (*selectedLeptonsAR)[l];
        delete (*selectedLeptonsAR_copy)[l];
      }
      selectedLeptonsAR->clear();
      selectedLeptonsAR->shrink_to_fit();
      delete selectedLeptonsAR;
      selectedLeptonsAR_copy->clear();
      selectedLeptonsAR_copy->shrink_to_fit();
      delete selectedLeptonsAR_copy;
      continue;
    }
